...
// 根据不同字段搜索员工
const searchEmployee = async (field, value) => {
    if (!value) {
    ...
    switch (field) {
        case 'id':
            await store.dispatch('findEmployeeById', value);
            searchResults.value = store.state.employees.foundEmployee;
            break;
        case 'name':
            await store.dispatch('findEmployeeByName', value);
            searchResults.value = store.state.employees.foundEmployee;
            break;
        case 'position':
            await store.dispatch('findEmployeeByPosition', value);
            searchResults.value = store.state.employees.foundEmployee;
            break;
        // 添加更多字段的搜索逻辑
        default:
...
const performSearch = () => {
    searchEmployee(searchField.value, search.value);
}
</script>