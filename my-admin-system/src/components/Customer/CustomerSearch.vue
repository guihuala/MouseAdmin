...
const searchCustomer = async (field, value) => {
    ...
    switch (field) {
        case 'id':
            await store.dispatch('findCustomerById', value);
            searchResults.value = store.state.customers.foundCustomer;
            break;
        case 'name':
            await store.dispatch('findCustomerByName', value);            
            searchResults.value = store.state.customers.foundCustomer;         
            break;
        case 'address':
            await store.dispatch('findCustomerByAddress', value);
            searchResults.value = store.state.customers.foundCustomer;
            break;
        // 添加更多字段的搜索逻辑
        default:
...
const performSearch = () => {
    searchCustomer(searchField.value,search.value);
}
</script>